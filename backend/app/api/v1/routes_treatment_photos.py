...
async def create_api_v1_treatment_photos(
    ...
    result = await service.create_treatment_photo(db, request_dict)
    if not result:
        raise HTTPException(
...
async def list_api_v1_treatment_photos(
    ...
    db: AsyncSession = Depends(get_db)
) -> treatment_photos_response_28:
    """시술 사진 목록"""
    service = TreatmentPhotosService()
    photos = await service.list_treatment_photos(db, treatment_id=treatment_id, session_id=session_id)
...
async def delete_api_v1_treatment_photos_by_id(
    ...
    """시술 사진 삭제"""
    service = TreatmentPhotosService()
    success = await service.delete_treatment_photo(db, id)
    if not success:
        raise HTTPException(

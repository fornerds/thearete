...
async def create_api_v1_skin_measurements(
    ...
    result = await service.create_skin_measurement(db, request_dict)
    if not result:
        raise HTTPException(
...
async def list_api_v1_skin_measurements(
    ...
    db: AsyncSession = Depends(get_db)
) -> skin_measurements_response_23:
    """회차별 측정 데이터 목록"""
    service = SkinMeasurementsService()
    measurements = await service.list_skin_measurements(db, session_id=session_id)
...
async def delete_api_v1_skin_measurements_by_id(
    ...
    """측정 데이터 삭제"""
    service = SkinMeasurementsService()
    success = await service.delete_skin_measurement(db, id)
    if not success:
        raise HTTPException(
